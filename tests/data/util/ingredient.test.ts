...
Deno.test(`ingredient.parse`, () => {
  const tests: {
    ...
    {
      raw: "1 cup of rice",
      expected: { amount: "1cup", amountType: "full", description: "of rice" },
    },
    {
      raw: "10g sugar",
      expected: { amount: "10g", amountType: "full", description: "sugar" },
    },
    {
      raw: "10-20g Zucker",
      expected: {
    ...
    {
      raw: "salt",
      expected: { amount: undefined, amountType: "empty", description: "salt" },
    },
  ];
...
Deno.test(`ingredient.parse with unknown unit (without postprocessing)`, () => {
  ...
  assertEquals(ingredient.parse("2 TL Salz"), {
    amount: "2",
    amountType: "unitless",
    description: "TL Salz",
  });
...
Deno.test(`ingredient.parseMany sort modes`, () => {
  const types = {
    full: {
      raw: "10g salt",
      parsed: { amount: "10g", amountType: "full", description: "salt" },
    },
    unitless: {
      raw: "10 tomatoes",
      parsed: { amount: "10", amountType: "unitless", description: "tomatoes" },
    },
    empty: {
      raw: "salt",
      parsed: { amount: undefined, amountType: "empty", description: "salt" },
    },
  };
  const permutations = [
...
Deno.test(`ingredient.parse amount calculation`, () => {
  ...
  for (const { raw, recipeYield, portions, expectedAmount } of tests) {
    assertEquals(
      ingredient.parse(raw, recipeYield, portions).amount,
      expectedAmount,
    );
  }
});
