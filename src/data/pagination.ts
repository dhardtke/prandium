...
export class PaginationBuilder<T> {
    ...
    public constructor(totalItems: number, currentPage: number = 1, pageSize: number = 10, maxPages: number = 10) {
    ...
    private static buildPage(url: URL, page: number): Page {
        return ({number: page, url: PaginationBuilder.buildPageUrl(url, page)});
    }

    public build(items: (limit: number, offset: number) => T[], currentUrl: URL): Pagination<T> {
        const url = new URL(currentUrl.toString());
        const pages = Array.from(Array((this.endPage + 1) - this.startPage).keys())
...
export class Pagination<T> {
    ...
    public constructor(pageSize: number, currentPage: number, totalItems: number, totalPages: number, firstPage: number, lastPage: number, startIndex: number,
                       endIndex: number, previousPage?: Page, nextPage?: Page, pages: Page[] = [], items: T[] = []) {
        this.pageSize = pageSize;
        this.currentPage = currentPage;
        this.totalItems = totalItems;
