      ...
      <div class="row gy-3">
        ...
        <div class="col-sm-auto d-flex align-items-center">
          <div class="d-flex border-end me-2 pe-2">
            <button type="button" data-hook="up" class="btn btn-sm btn-outline-secondary me-1" title="${e(t("up"))}" ${!i && "disabled"}>
              ${Icon("arrow-up")}
            </button>
            <button type="button" data-hook="down" class="btn btn-sm btn-outline-secondary" title="${e(t("down"))}" ${last && "disabled"}>
              ${Icon("arrow-down")}
            </button>
          </div>
          <button type="button" data-hook="delete" class="btn btn-sm btn-outline-danger me-1" title="${e(t("delete"))}">
            ${Icon("trash")}
          </button>
        </div>
      </div>
...
export const RecipeEditTemplate = (recipe?: Recipe) =>
  Page(recipe ? recipe.title : t("create"))(html`
    ...
    <form method="POST" autocomplete="off" enctype="multipart/form-data" id="edit-form">
            ...
            <div class="card-footer mt-auto">
              <div class="d-flex justify-content-between flex-wrap align-items-center">
                ...
                <button type="button" ${!recipe || !recipe.thumbnail ? " disabled" : ""} class="btn btn-sm btn-danger"
                        title="${e(t("recipe.form.delete_thumbnail"))}" data-hook="delete">
                  ${Icon("trash")}
                </button>
              </div>
            </div>
      ...
      ${arrayField(t("recipe.ingredients.title"), t("recipe.form.create_ingredient"), "ingredients", false, recipe?.ingredients)}
      ${arrayField(t("recipe.instructions"), t("recipe.form.create_instruction"), "instructions", true, recipe?.instructions)}
